...
class SuperBizAgentApp {
    ...
    initializeElements() {
        ...
        this.aiOpsModalClose = document.getElementById('aiOpsModalClose');
        this.aiOpsQuestion = document.getElementById('aiOpsQuestion');
        this.aiOpsModalCancel = document.getElementById('aiOpsModalCancel');
        this.aiOpsModalConfirm = document.getElementById('aiOpsModalConfirm');
    ...
    bindEvents() {
        ...
        if (this.aiOpsSidebarBtn) {
            this.aiOpsSidebarBtn.addEventListener('click', () => this.openAIOpsModal());
        }

        // AI Ops 模态框事件
        if (this.aiOpsModalClose) {
            this.aiOpsModalClose.addEventListener('click', () => this.closeAIOpsModal());
        }
        if (this.aiOpsModalCancel) {
            this.aiOpsModalCancel.addEventListener('click', () => this.closeAIOpsModal());
        }
        if (this.aiOpsModalConfirm) {
            this.aiOpsModalConfirm.addEventListener('click', () => this.confirmAIOps());
        }
        // 点击模态框外部关闭
        if (this.aiOpsModal) {
            this.aiOpsModal.addEventListener('click', (e) => {
                if (e.target === this.aiOpsModal) {
                    this.closeAIOpsModal();
                }
            });
        }
        ...
        if (this.refreshFilesBtn) {
            this.refreshFilesBtn.addEventListener('click', () => this.loadKnowledgeBaseFiles());
        }
    }
    ...
    updateUI() {
        ...
        if (this.messageInput) {
            ...
            this.messageInput.placeholder = '问问知微智能助手';
        }
    }
    ...
    async sendQuickMessage(message) {
        ...
        try {
            ...
            // 统一响应格式：检查 data.code 判断请求是否成功
            if (data.code === 200) {
                // data.data 是 ChatRespDTO 对象
                const chatResp = data.data;
                
                if (chatResp) {
                    // 更新会话ID
                    if (chatResp.conversationId) {
                        this.sessionId = chatResp.conversationId;
                    }
                ...
                while (true) {
                    ...
                    for (const line of lines) {
                        ...
                        if (line.startsWith('id:')) {
                            ...
                            // 处理 SSE 数据 - 适配后端 ChatRespDTO 格式
                            try {
                                const chatResp = JSON.parse(rawData);
                                console.log('[SSE调试] 解析JSON成功:', chatResp);

                                if (chatResp && chatResp.type) {
                                    const type = chatResp.type;
                                    const answer = chatResp.answer || '';
                                    const conversationId = chatResp.conversationId;
                                    const details = chatResp.details;
                                    ...
                                    if (type === 'content') {
                                        ...
                                        console.error('[SSE调试] 收到错误:', answer);
                                        if (assistantMessageElement) {
                                            const messageContent = assistantMessageElement.querySelector('.message-content');
                                            messageContent.innerHTML = this.renderMarkdown('错误: ' + (answer || '未知错误'));
                                        }
                                        return;
                                    }
                                } else {
                                    // 不是标准 ChatRespDTO 格式，尝试兼容处理
    ...
    // 发送智能运维请求（SSE 流式模式）
    async sendAIOpsRequest(loadingMessageElement, query) {
        try {
            const response = await fetch(`${this.apiBaseUrl}/ai_ops/troubleshoot`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                            ...
                            const processJsonMessages = (data) => {
                                ...
                                if (matches && matches.length > 0) {
                                    ...
                                    for (const jsonStr of matches) {
                                        try {
                                            const chatResp = JSON.parse(jsonStr);
                                            if (chatResp && chatResp.type) {
                                                if (chatResp.type === 'content') {
                                                    fullResponse += chatResp.answer || '';
                                                    // 更新会话ID
                                                    if (chatResp.conversationId) {
                                                        this.sessionId = chatResp.conversationId;
                                                    }
                                                ...
                                                } else if (chatResp.type === 'done') {
                                                    console.log('AI Ops 流完成，最终内容长度:', fullResponse.length);
                                                    this.updateAIOpsMessage(loadingMessageElement, fullResponse, chatResp.details || []);
                                                    return true;
                                                } else if (chatResp.type === 'error') {
                                                    throw new Error(chatResp.answer || '智能运维分析失败');
                                                }
                                            }
                                        } catch (e) {
    ...
    openAIOpsModal() {
        ...
        if (this.aiOpsModal) {
            this.aiOpsModal.classList.add('active');
            // 聚焦到输入框
            setTimeout(() => {
                if (this.aiOpsQuestion) {
                    this.aiOpsQuestion.focus();
                }
            }, 100);
        }
    }
    ...
    closeAIOpsModal() {
        if (this.aiOpsModal) {
            this.aiOpsModal.classList.remove('active');
            // 清空输入框
            if (this.aiOpsQuestion) {
                this.aiOpsQuestion.value = '';
            }
        }
    }
    ...
    // 确认 AI Ops 诊断
    async confirmAIOps() {
        if (!this.aiOpsQuestion) return;

        const question = this.aiOpsQuestion.value.trim();
        if (!question) {
            this.showNotification('请描述您遇到的运维问题', 'warning');
            return;
        }
        ...
        try {
            await this.sendAIOpsRequest(loadingMessage, question);
        } catch (error) {
    ...
    async loadKnowledgeBaseFiles() {
        try {
            ...
            if (data.code === 200 && data.data) {
                this.renderKnowledgeBaseFiles(data.data);
            } else {
                throw new Error(data.message || '获取文件列表失败');
            }
        } catch (error) {
            console.error('获取知识库文件列表失败:', error);
            this.renderKnowledgeBaseFiles([]);
        }
    }

    // 渲染知识库文件列表
    renderKnowledgeBaseFiles(files) {
        if (!this.knowledgeBaseList) {
            return;
        }
        ...
        files.forEach(file => {
            ...
            // 根据文件类型选择图标
            const iconSvg = this.getFileIcon(extension);
    ...
    getFileIcon(extension) {
        const icons = {
            ...
            'md': `<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8l-6-6z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                <path d="M14 2v6h6M12 18l-2-2-2 2M12 10v8M8 10h8" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>`,
